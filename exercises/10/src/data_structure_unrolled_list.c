...
static void to_iter(unrolled_linked_list_t *m, unrolled_linked_list_i *i) {
    if (m == NULL) {
        ...
        *i = (unrolled_linked_list_i){
            .curr_chunk = m->chunks, .curr_elem_idx = 0, .manager = m};
    }
}
...
static char **get_chunk_pointer(unrolled_linked_list_i *i) {
    return ((char **)i->curr_chunk +
            i->manager->chunk_size * i->manager->element_size);
}
...
static bool is_current_chunk_pointer(unrolled_linked_list_i *i) {
    return ((i->curr_elem_idx % i->manager->chunk_size) == 0) &&
           (i->curr_elem_idx != 0);
}
...
static void *peek(unrolled_linked_list_i *i) {
    ...
    if (i->curr_elem_idx >= i->manager->num_elem) return NULL;
    ...
    char *chunk_pointer =
        *((char **)(i->curr_chunk +
                    (i->manager->chunk_size * i->manager->element_size)));
    // Return NULL if the chunk_pointer is NULL, otherwise return a reference to
    // the first element in the next chunk.
...
static void *next(unrolled_linked_list_i *i) {
    ...
    // Return reference to the start of the next chunk which equals the first
    // element.
    return i->curr_chunk;
}
...
static void insert_before_inner(char *curr_chunk, size_t element_size,
                                size_t num_elem, size_t chunk_size) {
    char **chunk_pointer = (char **)curr_chunk + element_size * chunk_size;
    if (*chunk_pointer == NULL) {
        // Check if the current chunk is full.
        if (num_elem % chunk_size == 1) {
            *chunk_pointer = (char *)malloc(
                chunk_size * element_size * sizeof(char) + sizeof(void *));
            if (*chunk_pointer == NULL) return;

            char **next_chunk_pointer =
                (char **)*chunk_pointer + element_size * chunk_size;
            *next_chunk_pointer = NULL;
            // Copy the last element of current chunk into the first element of
            // the next chunk.
            memcpy(*chunk_pointer, curr_chunk + (chunk_size - 1) * element_size,
                   element_size);
            // Move all elements in the current chunk forward one.
            memmove(curr_chunk + element_size, curr_chunk,
                    (chunk_size - 1) * element_size);
        } else {
            // Move all elements in the current chunk forward one.
            memmove(curr_chunk + element_size, curr_chunk,
                    ((num_elem - 1) % chunk_size) * element_size);
        }
    } else {
        // Move all elements in following chunks forward one.
        insert_before_inner(*chunk_pointer, element_size, num_elem, chunk_size);
        // Copy the last element of current chunk into the first element of the
        // next chunk.
        memcpy(*chunk_pointer, curr_chunk + element_size * (chunk_size - 1),
               element_size);
        // Move all elements in the current chunk forward one.
        memmove(curr_chunk + element_size, curr_chunk,
                (chunk_size - 1) * element_size);
...
static void insert_before(unrolled_linked_list_i *i, void *value) {
    ...
    char **chunk_pointer = get_chunk_pointer(i);
    if (is_current_chunk_pointer(i)) {
        // If there are no more chunks, create a new one and copy the desired
        // value into the first element.
        if (*chunk_pointer == NULL) {
            *chunk_pointer =
                (char *)malloc(i->manager->chunk_size *
                                   i->manager->element_size * sizeof(char) +
                               sizeof(void *));
            if (*chunk_pointer == NULL) return;
            ...
            *next_chunk_pointer = NULL;
            ...
            insert_before_inner(*chunk_pointer, i->manager->element_size,
                                i->manager->num_elem, i->manager->chunk_size);
        }
        ...
        char *curr_elem =
            i->curr_chunk + (i->curr_elem_idx % i->manager->chunk_size) *
                                i->manager->element_size;
        // Check if the current chunk is full.
        if ((i->manager->num_elem % i->manager->chunk_size) == 1 &&
            i->manager->num_elem != 1) {
            *chunk_pointer =
                (char *)malloc(i->manager->chunk_size *
                                   i->manager->element_size * sizeof(char) +
                               sizeof(void *));
            ...
            *next_chunk_pointer = NULL;
            ...
            // Copy the last element of current chunk into the first element of
            // the next chunk.
            memcpy(*chunk_pointer,
                   i->curr_chunk +
                       (i->manager->chunk_size - 1) * i->manager->element_size,
                   i->manager->element_size);
            memmove(curr_elem + i->manager->element_size, curr_elem,
                    (i->manager->chunk_size - 1) * i->manager->element_size);
        } else {
            // Calculate how many elements are still in this chunk after the
            // current element.
            size_t elems_in_chunk =
                (i->manager->num_elem - 1) % i->manager->chunk_size;
            // Since the chunk pointer is NULL, all elements are in the current
            // chunk.
            memmove(curr_elem + i->manager->element_size, curr_elem,
                    elems_in_chunk * i->manager->element_size);
        }
        ...
        char *curr_elem =
            i->curr_chunk + (i->curr_elem_idx % i->manager->chunk_size) *
                                i->manager->element_size;
        // Calculate how many elements are still in this chunk after the current
        // element. We know that the chunk must be full since the chunk pointer
        // is not NULL.
        size_t elems_in_chunk = i->manager->chunk_size -
                                (i->curr_elem_idx % i->manager->chunk_size) - 1;
        memmove(curr_elem + i->manager->element_size, curr_elem,
                elems_in_chunk * i->manager->element_size);
        memcpy(curr_elem, value, i->manager->element_size);
    }
...
static void remove_inner(char *curr_chunk, size_t element_size, size_t num_elem,
                         size_t chunk_size) {
    char **chunk_pointer = ((char **)curr_chunk + element_size * chunk_size);
    // Check if the iterator is in the last allocated chunk.
    if (*chunk_pointer == NULL) {
        // Recursion ends here after all elements are moved back one.
        // The previos recursive call already copied the first element from this
        // chunk.
        memmove(curr_chunk, &curr_chunk[element_size],
                (num_elem % chunk_size) * element_size);
    } else {
        // Move all elements in the current chunk back one.
        memmove(curr_chunk, &curr_chunk[element_size],
                (chunk_size - 1) * element_size);
        // Copy first element of next chunk into the last element of the current
        // chunk.
        memcpy(&curr_chunk[element_size * (chunk_size - 1)], *chunk_pointer,
               element_size);
...
static void *remove_elem(unrolled_linked_list_i *i) {
    ...
    // TODO: This returns a reference to the element that will be overridden by
    // the memmove. This would probably have to be malloc'd.
    char *curr_elem = (char *)peek(i);
    if (is_current_chunk_pointer(i)) {
        // NOTE: chunk_pointer cannot be null here, since the index of the
        // current element is less than the total amount of elements.
        //       This implies that there are elements after this, implying that
        //       another chunk has to exist.
        char **chunk_pointer = get_chunk_pointer(i);
        char **next_chunk_pointer =
            (char **)(*chunk_pointer +
                      i->manager->chunk_size * i->manager->element_size);
        ...
        // Check if the next chunk is the last chunk and will now be empty.
        if ((*next_chunk_pointer == NULL) &&
            ((i->manager->num_elem % i->manager->chunk_size) == 0)) {
            free(*chunk_pointer);
            *chunk_pointer = NULL;
        } else {
            remove_inner(*chunk_pointer, i->manager->element_size,
                         i->manager->num_elem, i->manager->chunk_size);
        }
    }
    // Check if the iterator is in the last allocated chunk.
    else if (in_last_chunk(i)) {
        memmove(curr_elem, curr_elem + i->manager->element_size,
                (i->manager->num_elem - i->curr_elem_idx) *
                    i->manager->element_size);
    } else {
        char **chunk_pointer = get_chunk_pointer(i);
        char **next_chunk_pointer =
        ...
        // Copy first element of next chunk into the last element of the current
        // chunk.
        memcpy(&i->curr_chunk[i->manager->element_size *
                              (i->manager->chunk_size - 1)],
               chunk_pointer, i->manager->element_size);
        ...
        if ((*next_chunk_pointer == NULL) &&
            ...
            *chunk_pointer = NULL;
            ...
            remove_inner(i->curr_chunk, i->manager->element_size,
                         i->manager->num_elem, i->manager->chunk_size);
        }
    }
...
int init_chunks(unrolled_linked_list_t *m, size_t element_size,
                size_t chunk_size) {
    if (m == NULL) return -1;

    char *chunk = (char *)malloc(chunk_size * element_size * sizeof(char) +
                                 sizeof(void *));
...
void *init_data_structure(size_t size) {
    unrolled_linked_list_data_structure *d =
        malloc(sizeof(unrolled_linked_list_data_structure));
    if (d == NULL) {
        // Handle memory allocation failure
        return NULL;
    }

    if (init_chunks(&d->t, ELEM_SIZE, CHUNK_SIZE) != 0) {
...
void destroy_data_structure(void *data_structure) {
    unrolled_linked_list_data_structure *d =
        (unrolled_linked_list_data_structure *)data_structure;

    // Free memory for each allocated chunk
    char *curr_chunk = d->t.chunks;
    char **next_chunk_pointer;
    while (curr_chunk != NULL) {
        next_chunk_pointer =
            (char **)(curr_chunk + d->t.chunk_size * d->t.element_size);
        char *temp = curr_chunk;
        curr_chunk = *next_chunk_pointer;
        free(temp);
    }
...
void write_element(void *data_structure, size_t index, element elem) {
    ...
    for (size_t j = 0; j < index; j++) {
        if (next(i) == NULL) {
            // If index is out of bounds, reset iterator to start
            to_iter(&d->t, i);
            return;
        }
    }

    // Write the element at the current position of the iterator
    write_to_next(i, &elem);
}
...
void insert_element(void *data_structure, size_t index, element elem) {
    ...
    // Insert the element before the current position of the iterator
    insert_before(i, &elem);
}
...
element *read_element(void *data_structure, size_t index) {
    ...
    for (size_t j = 0; j < index; j++) {
        if (next(i) == NULL) {
            // If index is out of bounds, reset iterator to start and return a
            // default element
            to_iter(&d->t, i);
            element default_elem = {0};
            return NULL;
        }
    }

    // Return the element at the current position of the iterator
    element *elem = (element *)next(i);
    return elem;
}
